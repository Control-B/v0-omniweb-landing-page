      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <DashboardCard className="flex min-h-[240px] flex-col">
          ...
          <dl className="mt-5 space-y-3 text-[15px] text-slate-600">
            <div className="dashboard-card-muted flex items-center justify-between gap-3 rounded-[18px] px-4 py-3.5"><dt>Status</dt><dd className="font-semibold text-slate-900 capitalize">{billingStatus.subscriptionStatus}</dd></div>
            <div className="dashboard-card-muted flex items-center justify-between gap-3 rounded-[18px] px-4 py-3.5"><dt>Trial days left</dt><dd className="font-semibold text-slate-900">{billingStatus.isTrialActive ? billingStatus.daysLeft ?? 0 : "—"}</dd></div>
            <div className="dashboard-card-muted flex items-center justify-between gap-3 rounded-[18px] px-4 py-3.5"><dt>Renewal date</dt><dd className="font-semibold text-slate-900">{formatDate(billingStatus.renewalDate)}</dd></div>
          </dl>
          <a href="#billing-plans" className="dashboard-primary-button mt-auto inline-flex h-12 items-center justify-center rounded-2xl px-5 text-[15px] font-semibold transition hover:opacity-95">Choose a plan</a>
        </DashboardCard>
        ...
        <DashboardCard className="flex min-h-[240px] flex-col">
          ...
          <button type="button" disabled className="dashboard-secondary-button mt-auto inline-flex h-12 items-center justify-center rounded-2xl px-5 text-[15px] font-semibold text-slate-400">Add payment method</button>
        </DashboardCard>
